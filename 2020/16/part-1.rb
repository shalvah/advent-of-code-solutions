  ...
  when ""
    ...
    when "fields"
      ...
      ranges.each do |range|
        ...
        (min..max).each do |value|
          taken_numbers[value] ||= []
          taken_numbers[value] << match[:field] # Right now, we don't need to store the field names, but it might be useful for part 2
        end
      end
    when "your_ticket"
      # Do nothing for now
    when "nearby_tickets"
      ...
      field_values.each do |number|
        invalids << number.to_i if taken_numbers[number] == nil
      end
    end
  end
end
