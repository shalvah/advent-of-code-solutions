...
def find(moons, steps)
  steps.times do
    moons.each_with_index do |current_moon, index|
      velocity = current_moon[:velocity]
...
energies = moons.sum do |moon_data|
  potential = moon_data[:position].sum {|(dimension, coordinate)| coordinate.abs}
  kinetic = moon_data[:velocity].sum {|(dimension, coordinate)| coordinate.abs}
  potential * kinetic
end
