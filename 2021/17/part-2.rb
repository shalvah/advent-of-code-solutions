...
winning_velocities = []
# Guesstimates!
upper_y_bound = ($y_range[1] - $y_range[0]).abs * 10
lower_y_bound = -($y_range[1] - $y_range[0]).abs * 10

matching_x.each do |try_x|
  (lower_y_bound..upper_y_bound).each do |y|
    v = [try_x, y]
    pos = [0, 0]
    positions = []
